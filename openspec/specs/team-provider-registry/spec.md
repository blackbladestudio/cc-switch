# team-provider-registry Specification

## Purpose

定义团队 Provider Registry 的远程配置格式（版本化 JSON），供管理员集中维护 endpoint、模型、协议等供应商参数；规定文档 schema、必填字段、版本校验规则，以及客户端从可配置 HTTP/HTTPS 地址拉取 registry 文档的行为与错误处理。
