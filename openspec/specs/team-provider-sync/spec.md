# team-provider-sync Specification

## Purpose

定义客户端如何将团队 Provider Registry 中的条目合并到本地 SQLite providers：ID 映射规则、字段级合并策略、本地密钥与个人字段保留、团队托管元数据、手动/自动同步调度与审计记录，以及冲突检测与解决机制。
